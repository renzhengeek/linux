...
static int discard_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (fd->supports_discard) {
		pr_alert("fake_discard: discards supported\n");
...
static void discard_status(struct dm_target *ti, status_type_t type,
...
MODULE_AUTHOR("Joe Thornber <joe.thornber@gmail.com>");
MODULE_LICENSE("GPL");
